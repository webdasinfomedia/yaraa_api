 ...
 */
var pusherInit = null;
var channelMessageSent = null;
var channelMessageList = null;
var MessageSentServiceName = 'message.sent';

function initializePusher(pusherKey, clusterKey, authEndpointUrl, authToken) {

    Pusher.logToConsole = false;
    pusherInit = new Pusher(pusherKey, {
        cluster: clusterKey,
...
function subscribeAndBindMessageSentChannel(channelName) {
    if (pusherInit != null) {
        channelMessageSent = pusherInit.subscribe(channelName);
        channelMessageSent.bind(MessageSentServiceName, function(data) {
            // Method to be dispatched on trigger.
            console.log("response : ");
            console.log(JSON.stringify(data));
            window.onMessageSend(JSON.stringify(data));
        });

    } else {
        console.log("pusher not init");
    }
}
function subscribeMessageListChannel(channelName, isCallback, currentUserEmail) {
    if (pusherInit != null) {
        channelMessageList = pusherInit.subscribe(channelName);

        if(!isCallback){
        return;
        }

        channelMessageList.bind("pusher:subscription_succeeded", function(members) {
            // Method to be dispatched on trigger.
            console.log("pusher:subscription_succeeded : ");
            console.log(JSON.stringify(members));
            members.each((member) => {
                // For example
                window.onMemberAdded(JSON.stringify(member));
              });
//            window.onGetAllMembers(JSON.stringify(data));
        });
        channelMessageList.bind("pusher:member_added", function(data) {
            // Method to be dispatched on trigger.
            console.log("pusher:member_added : ");
            console.log(JSON.stringify(data));
            window.onMemberAdded(JSON.stringify(data));
        });
        channelMessageList.bind("pusher:member_removed", function(data) {
            // Method to be dispatched on trigger.
            console.log("pusher:member_removed : ");
            console.log(JSON.stringify(data));
            window.onMemberRemoved(JSON.stringify(data));
        });
        channelMessageList.bind("client-message.list_update_" + currentUserEmail, function(data) {
            // Method to be dispatched on trigger.
            console.log("client-message.list_update : ");
            console.log(JSON.stringify(data));
            window.onPersonalListUpdate(JSON.stringify(data));
        });
        channelMessageList.bind("client-message.group_list_update", function(data) {
            // Method to be dispatched on trigger.
            console.log("client-message.group_list_update : ");
            console.log(JSON.stringify(data));
            window.onGroupListUpdate(JSON.stringify(data));
        });
...
function unsubscribeAndUnbindMessageListChannel(currentUserEmail) {
    if (pusherInit != null && channelMessageList != null) {
        channelMessageList.unbind("pusher:member_added");
        channelMessageList.unbind("pusher:member_removed");
        channelMessageList.unbind("client-message.group_list_update");
        channelMessageList.unbind("client-message.list_update_" + currentUserEmail);
        pusherInit.unsubscribe(channelMessageList);
    }
}

function triggerMessageListChannel(channelName, data) {
    if (pusherInit != null && channelMessageList != null) {
        channelMessageList.trigger(channelName, data);
    }
}

function unsubscribeAndUnbindMessageSentChannel() {
    if (pusherInit != null && channelMessageSent != null) {
        channelMessageSent.unbind(MessageSentServiceName);
        pusherInit.unsubscribe(channelMessageSent);
    }
}
